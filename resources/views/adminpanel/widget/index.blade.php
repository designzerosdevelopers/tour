            ...
            <div class="box-breadcrumb">
                <div class="breadcrumbs">
                    <ul>
                        ...
                        <li><span>Widgets list</span></li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="box-heading-right"> <a class="btn btn-brand-secondary" href="{{ route('widget.create') }}">
                    ...
                    <tbody>
                        @foreach ($tours as $tour)
                            <tr>
                                ...
                                <td>
                                    <div class="card-tour">
                                        {{-- <a href="#"><img src="" alt="Travilla"></a> --}}
                                        <a class="text-md-bold neutral-1000" href="#">{{ $tour->title }}</a>
                                    </div>
                                </td>
                                <td>
                                    <span class="text-md-medium neutral-500">{{ $tour->created_at }}</span>
                                </td>
                                <td class="text-center">
                                    <a class="btn btn-primary" href="{{ route('tours.show', $tour->slug) }}"
                                        style="padding: 2px 5px; font-size: 12px;">View</a>
                                    <a class="btn btn-secondary" href="{{ route('widget.edit', $tour->id) }} "
                                        style="padding: 2px 5px; font-size: 12px;">Edit</a>
                                    <form action="{{ route('widget.destroy', $tour->id) }}" method="POST"
    ...
    @if ($tours->hasPages())
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                ...
                @else
                    <li class="page-item">
                        <a class="page-link" href="{{ $tours->previousPageUrl() }}" aria-label="@lang('pagination.previous')">
                ...
                {{-- Pagination Elements --}}
                @foreach ($tours->links()->elements as $element)
                ...
                @if ($tours->hasMorePages())
                    <li class="page-item">
                        <a class="page-link" href="{{ $tours->nextPageUrl() }}" aria-label="@lang('pagination.next')">
                            <span aria-hidden="true">
                                <svg width="12" height="12" viewbox="0 0 12 12" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
