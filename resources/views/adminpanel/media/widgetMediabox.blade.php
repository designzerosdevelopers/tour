<div class="box-heading">
    <div class="box-heading-left">
        <div class="box-title">
            <h4 class="neutral-1000 mb-15">Single Image Select</h4>
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        mediaIndex(); // Call the mediaIndex function

        // Attach event listeners to each button
        document.querySelectorAll('.item-upload-image').forEach(button => {
            button.addEventListener('click', function() {
                const clickedButtonId = this.id; // Get the ID of the clicked button
                boxId = clickedButtonId.split('-')[
                2]; // Extract the ID part and assign it to boxId
                console.log('Selected Box ID:', boxId); // Log the box ID for verification
            });
        });
    });
    ...
    function addImageToPreview(image, box) {
        const imagePreviewContainer = document.getElementById('imagePreviewContainer'+box);
        ...
        <div class="box-image-preview position-relative">
            <input type="hidden" name="images[]" value="${image.id}" required>
    ...
    function toggleImageSelection(image) {
        ...
        if (selectedImage && selectedImage.id === image.id) {
            selectedImage = null;
            mediaItemDiv.style.border = '';
            mediaItemDiv.classList.add('border');
        } else {
            // Clear previous selection if another image is selected
            if (selectedImage) {
                const prevSelectedMediaItemDiv = document.querySelector(`img[data-id="${selectedImage.id}"]`)
                    .parentElement;
    ...
    document.getElementById('selectImagesBtn').addEventListener('click', function(event) {
        ...
        if (selectedImage) {
            addImageToPreview(selectedImage, boxId);
        }
