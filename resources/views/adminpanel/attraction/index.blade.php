...
@section('content')
    <div class="box-heading">
        <div class="box-heading-left">
            <div class="box-title">
                <h4 class="neutral-1000 mb-15">Attraction List</h4>
            </div>
            <div class="box-breadcrumb">
                <div class="breadcrumbs">
                    <ul>
                        <li><a class="icon-home" href="{{route('admin.index')}}">Dashboard</a></li>
                        <li><span>Attraction List</span></li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="box-heading-right"> <a class="btn btn-brand-secondary" href="{{ route('attraction.create') }}">
                    ...
                    <tbody>
                        @foreach ($attractions as $attraction)
                            <tr>
                                ...
                                <td>
                                    <div class="card-tour">
                                        {{-- <a href="#"><img src="{{ asset($attraction->image) }}" alt="Travilla"></a> --}}
                                        <a class="text-md-bold neutral-1000" href="#">{{ $attraction->title }}</a>
                                    </div>
                                </td>
                                <td>
                                    <span class="text-md-medium neutral-500">{{ $attraction->created_at }}</span>
                                </td>
                                <td>
                                    @if ($attraction->published === 1)
                                        <span class="btn btn-status">Published</span>
                                    @elseif ($attraction->published === 0)
                                        <span class="btn btn-status pending">Draft</span>
                                    @endif
                                </td>
                                <td class="text-center">
                                    <a class="btn btn-primary" href="{{ route('attractions.show', $attraction->slug) }}"
                                        style="padding: 2px 5px; font-size: 12px;">View</a>
                                    <a class="btn btn-secondary" href="{{ route('attraction.edit', $attraction->id) }} "
    ...
    @if ($attractions->hasPages())
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                ...
                @else
                    <li class="page-item">
                        <a class="page-link" href="{{ $attractions->previousPageUrl() }}" aria-label="@lang('pagination.previous')">
                ...
                {{-- Pagination Elements --}}
                @foreach ($attractions->links()->elements as $element)
                ...
                @if ($attractions->hasMorePages())
                    <li class="page-item">
                        <a class="page-link" href="{{ $attractions->nextPageUrl() }}" aria-label="@lang('pagination.next')">
                            <span aria-hidden="true">
                                <svg width="12" height="12" viewbox="0 0 12 12" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
